...
class UrbitRTCPeerConnection extends RTCPeerConnection {
  ...
  static async reconnect(params: Reconnect) {
    ...
    var app = params.app ? params.app : 'switchboard';
    var peer = await urbit.scry<string>({ app, path: `/call/${uuid}/peer` });
    var dap = await urbit.scry<string>({ app, path: `/call/${uuid}/dap` });

    var conn = new UrbitRTCPeerConnection(peer, dap, uuid, urbit, app);
    conn.reconnect = true;
    return conn;
  }
  ...
  closeWithError(err: Error) {
    this.close();
    console.error('Closed with error', err);
    this.onerror(err);
  }
...
export class UrbitRTCStateChangedEvent extends Event {
  // type = 'statechanged';
  uuid: string;
