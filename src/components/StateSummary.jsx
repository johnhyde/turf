...
import { bind, getDateString, vec2 } from 'lib/utils';
...
function Turf(props) {
  ...
  return (
    <>
      <p>
        {props.turf.size.x}x{props.turf.size.y}; offset:{' '}
        {props.turf.offset.x}x{props.turf.offset.y}
      </p>
    </>
  );
}
    ...
    <div class='overflow-y-auto'>
      ...
      <div>
        <input
          ...
          max='4'
          step='1'
          use:bind={[() => state.scaleLog, (s) => state.setScaleLog]}
        />
      </div>
      <h3>Current Turf:</h3>
      ...
      <ul>
        ...
        <li>
          <button onClick={[subToTurf, '~bindev-midlev-mindyr']}>
            sub to bindev
          </button>
        </li>
        <li>
          <button onClick={[subToTurf, '~fasdev-naltuc-ravteb']}>
            sub to fasdev
          </button>
        </li>
        <li>
      ...
      <input
        ...
        name='join'
        onKeyDown={(e) => {
          if (e.code == 'Enter') subToTurf(e.target.value);
        }}
      >
      </input>
      {/* <button onClick={} */}
